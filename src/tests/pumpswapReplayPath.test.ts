...
test('replay paths estimate gross and modeled net return from reserve snapshots', () => {
  ...
  assert.equal(report.paths[0].initialLiquiditySol, 100)
  assert.equal(report.paths[0].entryLiquiditySol, 100)
  assert.equal(report.paths[0].entryLiquidityGrowthPct, 0)
...
test('replay segments capture pre-entry momentum and crowding cohorts', () => {
  ...
  assert.equal(path.initialLiquiditySol, 100)
  assert.equal(path.entryLiquiditySol, 110)
  assert.equal(Math.round(path.entryLiquidityGrowthPct ?? 0), 10)
  assert.equal(Math.round(path.entryMomentumPct ?? 0), 22)
  assert.ok(segment)
  assert.ok(segment?.segment.includes('rent=yes'))
  assert.ok(segment?.segment.includes('initial_liquidity=75_to_125_sol'))
  assert.ok(segment?.segment.includes('entry_liquidity_growth=0_to_10_pct'))
