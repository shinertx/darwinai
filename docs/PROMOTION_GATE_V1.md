...
Before spending the first loop, the batch runner also scans recent `cyborg-canary-*.json` and `cyborg-dry-run-*.json` evidence for the same cyborg strategy config and canary size. If that exact config and size already produced a non-positive, incomplete, unflattened, or modeled-negative live/shadow result, the runner refuses to start. For dry-run blocking only, `liveSignalMaxAgeMs` is ignored so freshness metadata cannot hide a modeled-negative result from the same economic/scorer config. `CYBORG_PROMOTION_ALLOW_KNOWN_UNPROFITABLE=true` is a diagnostic override only; it must not be used as promotion proof.

The batch runner also requires at least one matching positive `cyborg-dry-run-*.json` shadow result for the same economic/scorer config and canary size before it starts spending. A paper candidate with no live-shadow positive sample is not enough to start the 20-loop funded batch.
